...
const TimelineContext = createContext<TimelineContext | null>(null);
...
function TimelineProvider({ context, force, className, hideRowHeader, children, ...props }: TimelineProps) {
...
function TimelineControl({ className, ...props }: React.ComponentProps<'div'>) {
    return (
        <div
            {...props}
...
// (optional) you can remove scroll-area and add class `overflow-x-auto` then replace `eRef` to `ref`
function TimelineContent({ className, style, children, ...props }: React.ComponentProps<'div'>) {
    const { ref } = useTimelineContext();
    return (
        <ScrollArea ref={ref}>
            <div
                {...props}
...
interface TimelineHeaderProps extends
    React.ComponentProps<'div'> {
    render?: (date: Date) => React.ReactNode
}

const TimelineHeader = memo(({ render, className, ...props }: TimelineHeaderProps) => {
    const { unit, units, isNotEmpty, hideRowHeader } = useTimelineContext();
            ...
            {units.map((date) => (
                <div
                    ...
                    className={cn(
                        ...
                        getWidth(unit),
                        inRange(unit, new Date(), date) && 'bg-primary/10',
                    )}
                >
                    {render ? render(date) : (
...
const TimelineRows = memo(({ className, children, ...props }: React.ComponentProps<'div'>) => {
    const { data, unit, units, field, isNotEmpty, hideRowHeader } = useTimelineContext();
...
interface TimelineDropCellProps extends
    Omit<React.ComponentProps<'div'>, 'onDrop'> {
    accept: string | number
    onDrop?: () => void
}

const TimelineDropCell = memo(({ onDrop, accept, className, children, ...props }: TimelineDropCellProps) => {
    const { unit } = useTimelineContext();
...
TimelineDropCell.displayName = "TimelineDropCell";

interface TimelineDragCellProps extends
    React.ComponentProps<'div'> {
    item: TData
}

const TimelineDragCell = memo(({ item, className, ...props }: TimelineDragCellProps) => {
...
TimelineDragCell.displayName = "TimelineDragCell";
...
export function useTimeline<T extends TData>({
    data,
    field,
    unit,
    dateRange
